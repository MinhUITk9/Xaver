...
class ByteHistogram(FeatureBaseType):

    name = 'histogram'
    dim = constant.BYTE_HISTOGRAM_DIM
...
class ByteEntropyHistogram(FeatureBaseType):
    name = 'byteentropy'
    dim = constant.BYTE_ENTROPY_HISTOGRAM_DIM

    def __init__(self, step=1024, window=2048):
        super(FeatureBaseType, self).__init__()
        self.window = window
        self.step = step
...
class SectionInfo(FeatureBaseType):
    name = 'section'
    dim = constant.SECTION_DIM
...
class ImportsInfo(FeatureBaseType):
    name = 'imports'
    dim = constant.IMPORT_TABLE_DIM
...
class ExportsInfo(FeatureBaseType):
    name = 'exports'
    dim = constant.EXPORT_TABLE_DIM
...
class GeneralFileInfo(FeatureBaseType):
    name = 'general'
    dim = constant.GENERAL_FILE_INFO_DIM
...
class HeaderFileInfo(FeatureBaseType):
    """ COFF header and optional header. """

    name = 'header'
    dim = constant.HEADER_FILE_INFO_DIM
...
class StringExtractor(FeatureBaseType):
    """ Extracts strings from raw byte stream """

    name = 'strings'
    dim = constant.STING_DIM
