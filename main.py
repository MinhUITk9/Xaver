...
from model import TrainDataset
from model import EvaluateTestSample

from vectorize import VectorDataset
...
def InitLog():
    logSys = logging.getLogger()
    logSys.setLevel(logging.INFO)

    log_formatter = logging.Formatter("%(asctime)s XAVER [%(levelname)-5.5s]  %(message)s", "%Y-%m-%d %H:%M:%S")

    # Write logs to file
    fileHandler = logging.FileHandler(os.path.join(os.getcwd(), 'xaver.log'))
    fileHandler.setFormatter(log_formatter)
    logSys.addHandler(fileHandler)

    # Write logs to console
    console = logging.StreamHandler()
    console.setFormatter(log_formatter)
    logSys.addHandler(console)
...
def DownloadDataset():
    ...
    logging.info('The dataset is downloaded and extracted')
